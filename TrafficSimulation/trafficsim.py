...
#     win.run(steps_per_update=STEPS_PER_UPDATE, vehicle_rate=None, weight=None)
...
class TrafficSim:
    def __init__(self):
        ...
        'vehicles':[
            # 1st Lane
            [2*4, {'path': [0, 8, 6]}],
            [2*4, {'path': [0, *road(NUM_OF_ROADS), 5]}],
            # [2*4, {'path': [0, *road(24+n), 7]}],

            [2*4, {'path': [1, 9, 7]}],
            [2*4, {'path': [1, *road(NUM_OF_ROADS+2*n), 6]}],
            # [1*4, {'path': [1, *road(24+3*n), 4]}],

            [3*4, {'path': [2, 10, 4]}],
            [3*4, {'path': [2, *road(NUM_OF_ROADS+4*n), 7]}],
            # [1*4, {'path': [2, *road(24+5*n), 5]}],

            [4*4, {'path': [3, 11, 5]}],
            [4*4, {'path': [3, *road(NUM_OF_ROADS+6*n), 4]}],
            # [2*4, {'path': [3, *road(24+7*n), 6]}],

            # 2nd Lane
            [2*4, {'path': [12, 20, 18]}],
            # [2*4, {'path': [12, *road(24+8*n), 17]}],
            [2*4, {'path': [12, *road(NUM_OF_ROADS+9*n), 19]}],

            [2*4, {'path': [13, 21, 19]}],
            # [2*4, {'path': [13, *road(24+10*n), 18]}],
            [2*4, {'path': [13, *road(NUM_OF_ROADS+11*n), 16]}],

            [3*4, {'path': [14, 22, 16]}],
            # [2*4, {'path': [14, *road(24+12*n), 19]}],
            [3*4, {'path': [14, *road(NUM_OF_ROADS+13*n), 17]}],

            [4*4, {'path': [15, 23, 17]}],
            # [2*4, {'path': [15, *road(24+14*n), 16]}],
            [4*4, {'path': [15, *road(NUM_OF_ROADS+15*n), 18]}],


            # 3rd Lane (no red light/turn left only)
            # [3*4, {'path': [24, 32, 30]}],
            # [2*4, {'path': [24, *road(NUM_OF_ROADS+16*n), 29]}],
            [3*4, {'path': [24, *road(NUM_OF_ROADS+17*n), 31]}],

            # [3*4, {'path': [25, 33, 31]}],
            # [2*4, {'path': [25, *road(NUM_OF_ROADS+18*n), 30]}],
            [3*4, {'path': [25, *road(NUM_OF_ROADS+19*n), 28]}],

            # [3*4, {'path': [26, 34, 28]}],
            # [2*4, {'path': [26, *road(NUM_OF_ROADS+20*n), 31]}],
            [4*4, {'path': [26, *road(NUM_OF_ROADS+21*n), 29]}],

            # [3*4, {'path': [27, 35, 29]}],
            # [2*4, {'path': [27, *road(NUM_OF_ROADS+22*n), 28]}],
            [6*4, {'path': [27, *road(NUM_OF_ROADS+23*n), 30]}]
        ...
        self.sim = sim
        self.win = Window(self.sim)
        self.win.zoom = 4
        
    
    
    def update_state(self, data, output=0):
        # Start simulation
        ...
        time = int(log['time']) // 2
        time = time % 4674
        cols = ['jl-magelang-masuk', 'jl-kyaimojo-masuk',
                'jl-diponegoro-masuk', 'jl-tentarapelajar-masuk']
        data = data[cols]
        new_state = data.loc[time ]
        new_state_a = new_state.to_numpy().sum()
        new_state_b = new_state.to_numpy().astype(int)
        config = (new_state_a, new_state_b, int(output))
        # self.win.new_config(config)
        # if(self.sim.isPaused == False):
        self.win.run(steps_per_update=self.STEPS_PER_UPDATE, config=config, vehicle_rate=None, weight=None)



    def get_log(self):
        return self.win.get_logging()
